...
function makeCard(card, large=false) {
  ...
  node.querySelector(".front-caption").textContent = card.name;
...
async function searchPeople() {
  ...
  try {
    ...
    if (!candidates.length) {
      ...
      manual.onclick = () => openEditor({name, birthday:"", photo:""});
    ...
    const manual = document.createElement("button");
    manual.type = "button"; manual.className = "secondary";
    manual.textContent = "None of these — enter manually";
    manual.onclick = () => openEditor({name, birthday:"", photo:""});
    results.appendChild(manual);
  } catch (err) {
...
function openCard(id) {
  ...
  $("largeCard").replaceWith(Object.assign(makeCard(card, true), {id:"largeCard"}));
...
$("photoUrl").addEventListener("input", e => {
  if (!uploadedDataUrl) $("previewImage").src = e.target.value || placeholder;
});
...
$("studyBtn").onclick = () => {
  ...
  $("studyBtn").textContent = document.body.classList.contains("study") ? "Show names" : "Study mode";
};
$("backupBtn").onclick = () => {
